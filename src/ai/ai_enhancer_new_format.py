...
class AIEnhancerNewFormat:
    ...
    def enhance_product(self, product: pd.Series, preserve_existing: bool = True, 
                       force_reprocess: bool = False) -> pd.Series:
        """
        ...
        if preserve_existing:
            has_short = str(result.get('shortDescription', '')) not in ['', 'nan', 'None']
    ...
    def enhance_dataframe(self, df: pd.DataFrame, force_reprocess: bool = False) -> pd.DataFrame:
        """
        ...
        for idx, row in result_df.iterrows():
            ...
            # Enhance product
            enhanced = self.enhance_product(row, preserve_existing=not force_reprocess, 
    ...
    def enhance_dataframe_with_stats(self, df: pd.DataFrame, 
        ...
        newly_processed = total_processed - (0 if force_reprocess else already_processed)
    ...
    def _call_ai_api(self, product: pd.Series) -> Dict:
        ...
        if str(product.get('shortDescription', '')) in ['', 'nan', 'None']:
            ...
            if name:
                enhancements['shortDescription'] = f"{name} - Professional quality product"
        ...
        if str(product.get('description', '')) in ['', 'nan', 'None']:
            name = str(product.get('name', ''))
            if name:
                enhancements['description'] = f"Detailed description for {name}. High-quality professional equipment suitable for commercial use."
