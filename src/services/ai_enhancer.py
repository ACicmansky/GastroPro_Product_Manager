    ...
    def _check_and_wait_for_quota(self, tokens_needed: int = 0):
        ...
        with self.calls_lock:
            ...
            if need_to_wait:
                ...
                if time_to_wait > 0:
                    logger.info(
                        f"Quota limit reached ({wait_reason}), waiting {time_to_wait:.1f} seconds until next minute..."
                    )
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                response = self.client.models.generate_content(
                    model=self.model_name, config=self.config, contents=user_prompt
                )
    ...
    def update_dataframe(
        ...
        for enhanced_product in enhanced_products:
            ...
            exact_matches = search_df[
                search_df["Kat. číslo"].astype(str).str.strip() == cat_num
            ]
            ...
            if len(exact_matches) == 1:
                ...
                best_match_idx = exact_matches.index[0]
                logger.warning(
                    f"Multiple exact matches for {cat_num}, using first: {best_match_idx}"
                )
            else:
                # Strategy 2: Fuzzy match on catalog number (handles minor variations)
                best_match_idx = self.find_best_match(
                    enhanced_product["Kat. číslo"], "Kat. číslo", search_df
                )
                if best_match_idx is not None:
                    logger.info(
                        f"Fuzzy match on catalog number for {cat_num}: {best_match_idx}"
                    )
                else:
                    # Strategy 3: Fuzzy match on product name (last resort)
                    best_match_idx = self.find_best_match(
                        enhanced_product["Názov tovaru"], "Názov tovaru", search_df
                    )
                    if best_match_idx is not None:
                        logger.info(
                            f"Fuzzy match on name for {enhanced_product['Názov tovaru']}: {best_match_idx}"
                        )
            ...
            if best_match_idx is not None:
                ...
                df.at[best_match_idx, "SEO kľúčové slová"] = enhanced_product[
                    "SEO kľúčové slová"
                ]
                # CRITICAL: Set tracking columns separately to preserve boolean type
                df.at[best_match_idx, "Spracovane AI"] = True
                df.at[best_match_idx, "AI_Processed_Date"] = datetime.now().strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
                updated_count += 1
            else:
                logger.error(
                    f"No match found for product {enhanced_product['Kat. číslo']} or {enhanced_product['Názov tovaru']}"
                )
    ...
    def process_dataframe(
        ...
        df["Spracovane AI"] = df["Spracovane AI"].apply(
            lambda x: (
                ...
                else False if str(x).strip().upper() in ["FALSE", "0", "NO", ""] else x
            )
        )
                    ...
                    if result:
                        ...
                        try:
                            ...
                            try:
                                ...
                                df_copy.to_csv(
                                    tmp_file, index=False, encoding="cp1250", sep=";"
                                )
                            except Exception:
                                # Fallback to utf-8 if cp1250 fails
                                df_copy.to_csv(
                                    tmp_file, index=False, encoding="utf-8", sep=";"
                                )

                            logger.info(
                                f"Saved incremental progress for batch {batch_num} to {tmp_file}"
                            )
                        except Exception as e:
                    ...
                    if df_copy[col].dtype == "object":
                        ...
                        df_copy[col] = df_copy[col].apply(
                            lambda x: "".join(
                                (
                                    c
                                    if c.encode("cp1250", errors="replace") != b"?"
                                    else " "
                                )
                                for c in x
                            )
        ...
        logger.info(
            f"AI enhancement completed. Processed {processed_count} products out of {total_products} eligible products"
        )
        statistics = {
