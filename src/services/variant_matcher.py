...
class ProductVariantMatcher:
    ...
    def __init__(
        ...
        extraction_config_path="variant_extractions.json",
    ):
        """
    ...
    def _load_extraction_config(self):
        ...
        try:
            with open(self.extraction_config_path, "r", encoding="utf-8") as f:
                config = json.load(f)
                self.log_progress(
                    f"Loaded extraction configuration from {self.extraction_config_path}"
                )
                return config
    ...
    def extract_base_name(self, name):
        ...
        base = re.sub(
            r"\s+\d+(?:[xX×]\d+)+(?:\s*mm)?(?:\s*cm)?(?:\s*l)?(?:\s|$)", " ", name
        )
        ...
        base = re.sub(
            r"\s*[-–]\s*\d+(?:[xX×]\d+)*(?:\s*mm)?(?:\s*cm)?(?:\s*l)?", "", base
        )
    ...
    def analyze_variants(self, df, generate_report=True):
        ...
        if "Kat. číslo rodiča" in df.columns:
            has_parent = df["Kat. číslo rodiča"].notna() & (
                df["Kat. číslo rodiča"] != ""
            )
        else:
        ...
        used_as_parent = []
        if "Kat. číslo rodiča" in df.columns:
            used_as_parent = df.loc[
                df["Kat. číslo rodiča"].notna() & (df["Kat. číslo rodiča"] != ""),
                "Kat. číslo rodiča",
            ].unique()
        not_parent = (
            ~df["Kat. číslo"].isin(used_as_parent)
            if len(used_as_parent) > 0
        ...
        # Extract base names
        products_to_process["base_name"] = products_to_process["Názov tovaru"].apply(
            self.extract_base_name
        )
        products_to_process = products_to_process[
            products_to_process["base_name"] != ""
        ]
        ...
        for i, (idx1, row1) in enumerate(product_data):
            ...
            for idx2, row2 in product_data[i + 1 :]:
                ...
                if len(base_name2) < 8:
                    continue
                len_ratio = min(len(base_name1), len(base_name2)) / max(
                    len(base_name1), len(base_name2)
                )
                if len_ratio < 0.5:
                    continue
        ...
        if groups_count > 0:
            self.log_progress(
                f"Detected {variants_count} variants in {groups_count} groups (analysis only)"
            )
        ...
        for group in group_data:
            ...
            for product in group.get("products", []):
                ...
                for idx in catalog_to_indices.get(cat, []):
                    if (
                        override
                        or result_df.at[idx, "Kat. číslo rodiča"] in ("", None)
                        or pd.isna(result_df.at[idx, "Kat. číslo rodiča"])
                    ):
                        result_df.at[idx, "Kat. číslo rodiča"] = parent
                        assigned += 1
        self.log_progress(
            f"Assigned parent catalog numbers to {assigned} products from group_data"
        )
        return result_df
    ...
    def _write_assignment_summary(self, summary: dict, filepath: str):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"Conflicts overridden: {len(summary.get('overridden_conflicts', []))}\n"
            )
            f.write(f"Conflicts skipped: {len(summary.get('skipped_conflicts', []))}\n")
            f.write(
                f"Unmatched parent tokens: {len(summary.get('unmatched_parents', []))}\n"
            )
            f.write(
                f"Unmatched product tokens: {len(summary.get('unmatched_products', []))}\n"
            )
            f.write(
                f"Ambiguous parent tokens: {len(summary.get('ambiguous_parents', []))}\n"
            )
            f.write(
                f"Ambiguous product tokens: {len(summary.get('ambiguous_products', []))}\n\n"
            )
    ...
    def assign_variants_from_report(
        self, df, report_path, override=True, generate_differences=True
    ):
        """
        ...
        def resolve_token(token):
            ...
            if not any(ch in token_norm for ch in ["*", "?", "x", "X"]):
                ...
                candidates = norm_to_originals.get(
                    self._normalize_for_match(token_norm), set()
                )
        ...
        for grp in group_data_raw:
            ...
            for p in grp.get("products", []):
                ...
                if not matches:
                    summary["unmatched_products"].append(
                        {"group_id": gid, "token": token}
                    )
                    continue
                if len(matches) > 1:
                    summary["ambiguous_products"].append(
                        {"group_id": gid, "token": token, "match_count": len(matches)}
                    )
                for cat in matches:
                    ...
                    name = (
                        result_df.at[idx0, "Názov tovaru"]
                        if idx0 is not None and "Názov tovaru" in result_df.columns
                        else p.get("name", "")
                    )
            ...
            if not parent_matches:
                if parent_token:
                    summary["unmatched_parents"].append(
                        {"group_id": gid, "token": parent_token}
                    )
                # Fallback: choose smallest catalog among expanded products
                if expanded_products:
                    chosen_parent = sorted(
                        [p["catalog_number"] for p in expanded_products],
                        key=natural_sort_key,
                    )[0]
            else:
                if len(parent_matches) > 1:
                    chosen_parent = parent_matches[0]
                    summary["ambiguous_parents"].append(
                        {
                            "group_id": gid,
                            "token": parent_token,
                            "match_count": len(parent_matches),
                            "selected": chosen_parent,
                        }
                    )
                else:
                    chosen_parent = parent_matches[0]
                ...
                for idx in catalog_to_indices.get(cat, []):
                    ...
                    if cur not in ("", None) and not pd.isna(cur):
                        if cur != chosen_parent:
                            if override:
                                summary["overridden_conflicts"].append(
                                    {
                                        "group_id": gid,
                                        "catalog": cat,
                                        "old_parent": cur,
                                        "new_parent": chosen_parent,
                                    }
                                )
        ...
        summary_path = os.path.join(
            self.report_dir, f"variant_assignment_summary_{timestamp}.txt"
        )
    ...
    def generate_report(self, group_data):
        ...
        report_filename = os.path.join(
            self.report_dir, f"product_variants_{timestamp}.txt"
        )
        ...
        with open(report_filename, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"Total variants: {sum(len(g['products']) for g in group_data)}\n\n"
            )

            # Write each group
            for group in group_data:
                f.write(
                    f"Group #{group['group_id']} - Parent catalog: {group['parent_catalog']}\n"
                )
                f.write("-" * 80 + "\n")
                ...
                for i, product in enumerate(group["products"], 1):
                    parent_indicator = (
                        "[PARENT]" if product["is_parent"] else "        "
                    )
                    f.write(
                        f"{i}. {parent_indicator} {product['catalog_number']} - {product['name']}\n"
                    )
    ...
    def generate_differences_report(self, df, group_data):
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_filename = os.path.join(
            self.report_dir, f"product_differences_{timestamp}.txt"
        )
            ...
            for group_idx, group in enumerate(group_data, 1):
                ...
                for product in group["products"]:
                    ...
                    if catalog in catalog_to_index:
                        ...
                        category = (
                            df.at[idx, "Hlavna kategória"]
                            if "Hlavna kategória" in df.columns
                            else ""
                        )
                        ...
                        width = (
                            df.at[idx, "Šírka"]
                            if "Šírka" in df.columns
                            and pd.notna(df.at[idx, "Šírka"])
                            and "Šírka" in columns_to_show
                            else ""
                        )
                        length = (
                            df.at[idx, "Dĺžka"]
                            if "Dĺžka" in df.columns
                            and pd.notna(df.at[idx, "Dĺžka"])
                            and "Dĺžka" in columns_to_show
                            else ""
                        )
                        height = (
                            df.at[idx, "Výška"]
                            if "Výška" in df.columns
                            and pd.notna(df.at[idx, "Výška"])
                            and "Výška" in columns_to_show
                            else ""
                        )
                        power = (
                            df.at[idx, "Výkon"]
                            if "Výkon" in df.columns
                            and pd.notna(df.at[idx, "Výkon"])
                            and "Výkon" in columns_to_show
                            else ""
                        )
                        volume = (
                            df.at[idx, "Objem"]
                            if "Objem" in df.columns
                            and pd.notna(df.at[idx, "Objem"])
                            and "Objem" in columns_to_show
                            else ""
                        )
                        variant = (
                            df.at[idx, "Variant"]
                            if "Variant" in df.columns
                            and pd.notna(df.at[idx, "Variant"])
                            and "Variant" in columns_to_show
                            else ""
                        )

                        # Check if product has any extracted differences
                        has_differences = any(
                            [width, length, height, power, volume, variant]
                        )
                ...
                for idx, product in enumerate(products_with_differences, 1):
                    ...
                    f.write(
                        f"{idx}. {parent_marker}{product['catalog']} - {product['name']}\n"
                    )
    ...
    def extract_product_differences(self, df, group_data):
        ...
        self.log_progress(
            f"Loaded extraction rules for {len(category_to_columns)} categories"
        )
        ...
        for group in group_data:
            ...
            for product in group["products"]:
                ...
                if catalog in catalog_to_index:
                    ...
                    short_desc = (
                        df.at[idx, "Krátky popis"]
                        if "Krátky popis" in df.columns
                        and pd.notna(df.at[idx, "Krátky popis"])
                        else ""
                    )

                    # Extract information based on the columns specified for this category
                    if any(
                        col in columns_to_extract for col in ["Šírka", "Dĺžka", "Výška"]
                    ):
                    ...
                    if "Variant" in columns_to_extract:
                        variant = self._extract_variant_differences(
            ...
            for pattern in power_patterns:
                ...
                if match:
                    ...
                    if len(groups) == 3:  # Multiple power units pattern
                        ...
                        normalized_val, normalized_unit = normalize_unit(
                            power_val, power_unit
                        )
    ...
    def _extract_variant_differences(self, full_name, base_name):
        ...
        if (
            difference and len(difference) <= 30
        ):  # Only return if difference is reasonable size
