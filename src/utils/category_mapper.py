...
def map_category(category, category_mappings, interactive_callback=None, product_name=None):
...
def get_category_suggestions(unmapped_category: str, existing_categories: List[str], top_n: int = 5) -> List[Tuple[str, float]]:
    ...
    for existing_cat in existing_categories:
        ...
        existing_parts = [p.strip() for p in existing_normalized.split('/') if p.strip()]
        ...
        token_similarity = fuzz.token_sort_ratio(unmapped_normalized, existing_normalized)
        
        # 3. Partial similarity - handles substring matches (20% weight)
        partial_similarity = fuzz.partial_ratio(unmapped_normalized, existing_normalized)
