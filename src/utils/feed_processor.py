...
def process_forgastro_category(
    category, category_manager=None, category_mapping_callback=None, product_name=None
):
    ...
    for item in root.findall(f".//{root_element_tag}"):
        ...
        for xml_key, csv_column in mapping.items():
            if "/" in xml_key:
                ...
                row[csv_column] = ", ".join(
                    [el.text.strip() for el in elements if el.text]
                )
            else:
        ...
        if feed_name == "forgastro":
            if "Hlavna kategória" in row and row["Hlavna kategória"]:
                ...
                row["Hlavna kategória"] = process_forgastro_category(
                    row["Hlavna kategória"],
                    category_manager,
                    category_mapping_callback,
                    product_name,
                )
            if product_desc_html:
                ...
                if "Krátky popis" in mapping.values() and params_text:
                    ...
                    row["Krátky popis"] = (
                        f"{current_short.strip()}\n{params_text}"
                        if current_short.strip()
                        else params_text
                    )
            ...
            product_name = row.get("Názov tovaru", "")
            processed_desc, processed_cat = process_gastromarket_text(
                description,
                category,
                category_manager,
                category_mapping_callback,
