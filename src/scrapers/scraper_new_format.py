...
class ScraperNewFormat:
    ...
    def _clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "code" in df.columns:
            ...
            if not duplicates.empty:
                ...
                logger.warning(
                    f"Found {len(duplicates)} products with duplicate codes"
                )
    ...
    def scrape_product_detail(self, product_url: str) -> Dict:
        ...
        try:
            ...
            image_columns = ['defaultImage', 'image', 'image2', 'image3', 'image4', 'image5', 'image6', 'image7']
...
class EnhancedScraperNewFormat(ScraperNewFormat):
    ...
    def scrape_products(
        self, category_urls: Optional[List[str]] = None, max_products: Optional[int] = None
    ) -> pd.DataFrame:
        """
        ...
        print("="*60)
        self._log_progress(f"Starting parallel scraping with {self.max_threads} threads...")
        ...
        with ThreadPoolExecutor(max_workers=self.max_threads) as executor:
            future_to_url = {
                executor.submit(self.get_product_urls, url): url for url in category_urls
            }
        ...
        self._log_progress(f"\n{'='*60}")
        self._log_progress(f"✓ Discovery complete: {len(unique_urls)} unique product URLs")
        self._log_progress(f"{'='*60}\n")
        ...
        with ThreadPoolExecutor(max_workers=self.max_threads) as executor:
            future_to_product = {
                executor.submit(self.scrape_product_detail, url): url for url in unique_urls
            }
                ...
                try:
                    ...
                    if product_data:
                        with self.lock:
                            ...
                            progress_pct = (len(scraped_products) / len(unique_urls)) * 100
                            self._log_progress(
        ...
        print("\n" + "="*60)
        self._log_progress(f"✓ PARALLEL SCRAPING COMPLETE: {len(result_df)} products scraped")
        print("="*60 + "\n")
        return result_df
