...
class DataMergerNewFormat:
    ...
    def __init__(self, config: Dict):
        ...
        self.config = config
        self.image_columns = [
            'defaultImage', 'image', 'image2', 'image3',
            'image4', 'image5', 'image6', 'image7'
        ]
    
    def merge(self, main_df: pd.DataFrame, feed_dfs: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        """
            ...
            for idx, row in feed_df.iterrows():
                ...
                if code in products_by_code:
                    ...
                    if image_count > existing['image_count']:
                        ...
                        print(f"  Updating images for {code}: {existing['image_count']} -> {image_count} images")
    ...
    def merge_with_stats(self, main_df: pd.DataFrame, 
                        feed_dfs: Dict[str, pd.DataFrame]) -> Tuple[pd.DataFrame, Dict]:
        """
        ...
        final_codes = set(result_df['code'].values) if 'code' in result_df.columns else set()
