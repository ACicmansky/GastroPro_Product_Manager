...
class PipelineNewFormat:
    ...
    def merge_with_main(self, main_df: pd.DataFrame, 
                       feed_dfs: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        """
    ...
    def run(self, xml_feeds: Dict[str, str], 
            output_file: Optional[str] = None,
            main_data_file: Optional[str] = None,
            apply_categories: bool = True,
            apply_transformation: bool = True) -> pd.DataFrame:
        """
    ...
    def run_with_stats(self, xml_feeds: Dict[str, str],
                      output_file: Optional[str] = None,
                      main_data_file: Optional[str] = None) -> Tuple[pd.DataFrame, Dict]:
        """
        ...
        stats = {
            ...
            'categories_mapped': (result_df['defaultCategory'].str.contains('Tovary a kategórie > ', na=False)).sum() if 'defaultCategory' in result_df.columns else 0
        }
