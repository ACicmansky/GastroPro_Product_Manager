...
class DataPipeline:
    ...
    def run(self, main_df, selected_categories, options):
        try:
            ...
            if selected_categories:
                filtered_df = main_df[
                    main_df["Hlavna kategória"].isin(selected_categories)
                ].copy()
            else:
            ...
            if (
                options.get("map_categories")
                and "Hlavna kategória" in filtered_df.columns
            ):
                self._log_progress("Applying category mappings...")
                category_mappings = self.category_manager.get_all()
                if category_mappings:
                    filtered_df = map_dataframe_categories(
                        filtered_df, category_mappings
                    )
            ...
            final_df, merge_stats = self._clean_and_merge(
                filtered_df, feed_dataframes, scraped_df
            )

            # Step 6: Variant Matching
            if options.get("variant_checkbox"):
                self._log_progress("Analyzing products for variant detection...")
                variant_matcher = ProductVariantMatcher(
                    progress_callback=self._log_progress
                )
                final_df, group_data = variant_matcher.identify_variants(
                    final_df, generate_report=True
                )
            ...
            ai_stats = {}
            if options.get("ai_enhancement_checkbox") and self.config.get(
                "ai_enhancement", {}
            ).get("enabled", False):
            ...
            self._log_progress(
                f"Final dataset ready with {len(final_df)} total products."
            )
            ...
            try:
                ...
                if root is not None:
                    ...
                    if df is not None and not df.empty:
                        ...
                        self._log_progress(
                            f"Parsed {len(df)} products from {feed_name}"
                        )
            except Exception as e:
    ...
    def _clean_and_merge(self, main_df, feed_dfs, scraped_df):
        ...
        def clean_df(df, name):
            ...
            if removed > 0:
                self._log_progress(
                    f"Removed {removed} products with empty catalog numbers from {name}"
                )
            return cleaned

        main_df_cleaned = clean_df(main_df, "main CSV")
        if "Kat. číslo rodiča" in main_df_cleaned.columns:
            main_df_cleaned["Kat. číslo rodiča"] = main_df_cleaned[
                "Kat. číslo rodiča"
            ].replace([0, "0"], "")
        ...
        final_df, merge_stats = merge_dataframes(
            main_df_cleaned, all_sources_to_merge, self.config["final_csv_columns"]
        )
        ...
        for col in final_df.columns:
            if col not in ai_tracking_columns and final_df[col].dtype == "object":
                final_df[col] = (
                    final_df[col].fillna("").astype(str).replace("nan", "").str.strip()
                )
        ...
        final_df.loc[final_df["Názov tovaru"].isin([None, ""]), "Názov tovaru"] = (
            final_df["Kat. číslo"]
        )
    ...
    def _run_ai_enhancement(self, df):
        ...
        try:
            ...
            def ai_progress_callback(processed, total):
                self._log_progress(
                    f"AI enhancement: {processed}/{total} products processed"
                )

            return ai_processor.process_dataframe(
                df, progress_callback=ai_progress_callback
            )
