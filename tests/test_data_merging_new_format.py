...
class TestNewFormatMerging:
    ...
    def test_merge_main_with_feed_new_format(self, config):
        ...
        feed_df = pd.DataFrame({
            ...
            'price': ['250.00', '300.00'],
            'defaultImage': ['http://feed.com/img2.jpg', 'http://feed.com/img3.jpg'],
...
class TestMultipleFeedMerging:
    ...
    def test_merge_three_feeds(self, config):
        ...
        merger = DataMergerNewFormat(config)
        result = merger.merge(pd.DataFrame(), {
            'feed1': feed1,
            'feed2': feed2,
            'feed3': feed3
        })
        ...
        assert len(result) == 4
        assert set(result['code'].values) == {'PROD001', 'PROD002', 'PROD003', 'PROD004'}
