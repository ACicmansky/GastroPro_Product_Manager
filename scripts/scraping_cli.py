...
def run_fast_scraper():
    ...
    try:
        ...
        df = get_scraped_products(
            use_fast_scraper=True, progress_callback=lambda x: print(x)
        )
    ...
    if custom_categories:
        try:
            ...
            if not df.empty:
                print(f"\n✅ Completed in {end_time - start_time:.2f} seconds")
                print(
                    f"Scraped {len(df)} products from {len(custom_categories)} custom categories"
                )
                save_to_csv(df)
                print("Data saved to topchladenie_products.csv")
            else:
