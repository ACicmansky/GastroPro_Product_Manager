...
class AtomerAPIClient:
        ...
        class FormParser(HTMLParser):
            ...
            def handle_endtag(self, tag):
                if tag == "textarea" and self.current_textarea:
                    self.form_data[self.current_textarea] = "".join(
                        self.textarea_content
                    )
                    self.current_textarea = None
                    self.textarea_content = []
    ...
    def get_product_categories(self, id_tovar: int) -> Optional[requests.Response]:
        ...
        """
        params = {"modul": "eshop-tovar-kategorie", "id_tovar": id_tovar}
    ...
    def update_product_category(
        self, id_tovar: int, id_kategorie: int, product_data: Dict[str, Any]
    ...
    def update_product_seo(
        self, id_tovar: int, id_kategorie: int, seo_data: Dict[str, str]
    ) -> Optional[requests.Response]:
        """
        ...
        try:
            response = self.session.post(
                self.BASE_URL, params=params, headers=headers, data=seo_data, timeout=30
            )
            response.raise_for_status()
            return response
        except requests.exceptions.RequestException as e:
...
def create_product_update_payload(
    ...
    **kwargs,
    ...
    required_fields = [
        ...
        "tbOldWeight",
        "tbCount",
        "cbExtranetSklad_sklad",
        "tbExtranetSklad_count",
        "cbExtranetSklad_jednotka",
        "tbExtranetSklad_nakupnaPrice",
        "cbExtranetSklad_nakupnaPriceType",
        "cbExtranetSklad_nakupnaDPH",
        "tbMinOrderCount",
        "tbMinOrderCountManual",
    ]
...
def create_seo_update_payload(
    title_sk: str, description_sk: str = "", keywords_sk: str = "", language: str = "SK"
) -> Dict[str, str]:
    """
...
def update_product_workflow(
    ...
    for key in sorted(existing_data.keys()):
        value_preview = (
            str(existing_data[key])[:50] if existing_data[key] else "(empty)"
        )
    ...
    print(
        f"  - tbShortTextSK: {product_data.get('tbShortTextSK', 'NOT FOUND')[:80]}..."
    )
    ...
    seo_data = create_seo_update_payload(
        title_sk=seo_title, description_sk=seo_description, keywords_sk=seo_keywords
    )
