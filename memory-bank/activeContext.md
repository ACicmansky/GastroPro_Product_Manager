...
- **Testing & QA**: **295 tests passing** in parallel with `pytest-xdist` (`test:fast` in ~20s), full branch coverage reporting with `pytest-cov` (`test:cov`).
- **Catalog Product Variant Pairing & Shoptet Compliance Engine**:
  - `CatalogVariantService` (`src/domain/products/variant_service.py`): Automated normalizer, Mebella base grouper, and dimension variant clusterer. Paired 2,130 products across 482 variant families with zero `.0` float artifacts, while leaving 7,582 standalone products empty in strict accordance with Shoptet specifications. Integrated directly into `SyncCatalogUseCase` (`apply_to_dataframe`) before export transformation and database persistence.
...
- **Multi-Tiered Data Verification Engine**:
  ...
  - `OEM Spec Adapters` (`src/scrapers/oem/`): Canonical manufacturer ground-truth extraction for all top brands (`ForcoldAdapter`, `LiebherrAdapter`, `StalgastAdapter`, `RobotCoupeAdapter`, `RollerGrillAdapter`, `HendiAdapter`) covering 4,389 products (45.2% of the catalog) for $0.00 with a unified registry (`get_all_oem_adapters()`, `find_oem_adapter()`).
  - `SpecPatcher` (`src/domain/specs/patcher.py`): Safe, synchronized patching of parameters and customer-facing HTML text (shortDescription, description, FAQ, metaDescription) with automatic SQLite backups and `--dry-run` simulation mode. Patched confirmed typos in `F840130` series, `F852412`, `F787025`, `F725001`, and `S786961`.
